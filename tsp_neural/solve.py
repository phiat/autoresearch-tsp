...
K_NEIGHBORS = 10           # baseline / harvest k-NN candidate count
K_NEIGHBORS_RANKED = 30    # expanded pool the MLP ranker selects from
K_USE_RANKED = 10          # # of top-scored candidates the ranker actually visits
...
def two_opt_sweep_ranked(tour, pos, xy, is_prime_f32, candidates, k_use,
    ...
    """2-opt sweep where the K candidates per ai are scored by the MLP, then
    only the top `k_use` (by score) are visited in score order, taking the
    first improving swap. One accept per ai per sweep.

    `candidates` is shape (n, K_in). When K_in > k_use the ranker effectively
    expands the pool (I3-style); when K_in == k_use it just reorders (I1)."""
    n = len(xy)
...
def solve(xy, is_prime, budget, harvest_bufs=None, ranked_weights=None):
    ...
    tree = cKDTree(xy)
    k_query = max(K_NEIGHBORS, K_NEIGHBORS_RANKED) + 1
    ...
    if harvest_bufs is not None:
        ...
        print(f"  running 2-opt (RANK, K_in={K_NEIGHBORS_RANKED} → top {K_USE_RANKED} by MLP score) ...")
