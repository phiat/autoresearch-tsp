...
def gls_update(tour, xy, candidates, edge_penalty):
    """Bump the penalty of the max-utility candidate edge in the current tour
    (utility = length / (1 + penalty)). GLS feature-selection rule."""
    n_tour = len(tour) - 1
    K = candidates.shape[1]
    max_util = 0.0
    max_a = -1
...
def run_local(tour, pos, xy, candidates, budget, edge_penalty, lam, max_outer=20):
    """Alternate GLS-augmented 2-opt and (plain) Or-{1..5} sweeps until convergence;
    bumps max-utility tour edge's penalty at the end."""
    total_2opt = 0
    total_or = 0
