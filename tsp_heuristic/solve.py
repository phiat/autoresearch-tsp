...
def ensemble_ils_loop(best_tour, best_cost, xy, candidates, is_prime, budget,
    ...
    """Run `workers` independent sequential ILS trajectories in parallel; return
    min(all). Worker 0 uses seed 0xBEEF (guaranteed X8 trajectory) so we never
    do worse than X8; workers 1..N-1 use random seeds for diversity."""
    ...
    sorted_results = sorted(enumerate(results), key=lambda x: x[1][0])
    print(f"  trajectory costs: " + ", ".join(
        f"w{i}={r[0]:.0f}" for i, r in sorted_results))
    cand_cost, cand_tour = sorted_results[0][1]
...
def solve(xy, is_prime, budget):
    ...
    if ILS_WORKERS > 1:
        # Ensemble: each worker runs full sequential ILS independently; take min.
        # Worker 0 pinned to 0xBEEF (guarantees X8 trajectory); rest random.
